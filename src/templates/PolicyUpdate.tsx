...
export const POLICY_UPDATE_STRINGS: PolicyUpdateStrings = {
  subject: (policyType, appName) =>
    `Important: Updates to our ${POLICY_LABELS[policyType]} — ${appName}`,
  heading: (policyType) => `We've updated our ${POLICY_LABELS[policyType]}`,
  intro:
    "We've made changes to keep our policies clear and up to date. Please take a moment to review the updated terms.",
  effectiveDate: (date) => `These changes take effect on ${date}.`,
  ctaLabel: 'Review Changes',
  continuedUseNote:
    'By continuing to use our service after the effective date, you agree to the updated policy.',
...
export const PolicyUpdate: EmailTemplateType<PolicyUpdateProps> = defineEmail<PolicyUpdateProps>({
  ...
  component: ({
    ...
    return (
      <EmailTemplate preview={s.subject(policyType, appName)} lang={locale} dir={dir} theme={theme}>
        ...
        <Text muted size="sm">
          {s.continuedUseNote}
          {supportEmail && <> {s.questionNote(supportEmail)}</>}
        </Text>
