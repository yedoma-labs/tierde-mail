...
export interface RegistrationConfirmationProps extends BaseTemplateProps<RegistrationConfirmationStrings> {
  name: string;
  dashboardUrl: string;
  email?: string;
}
...
export const RegistrationConfirmation: EmailTemplateType<RegistrationConfirmationProps> = defineEmail<RegistrationConfirmationProps>({
  ...
  component: ({ name, dashboardUrl, email, appName = 'Our App', locale, dir, strings, theme }) => {
    const s = { ...REGISTRATION_CONFIRMATION_STRINGS, ...strings };
    ...
    return (
      <EmailTemplate preview={s.subject(appName)} lang={locale} dir={dir} theme={theme}>
        ...
        {email && <Text muted size="sm">Account email: <strong>{email}</strong></Text>}
