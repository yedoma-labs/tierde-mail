...
const templates: Record<string, string> = {
  ...
  MaintenanceNotification_emergency: renderEmail(MaintenanceNotification.component({
    type: 'emergency',
    affectedServices: [{ name: 'API', impact: 'full_outage' }, { name: 'Dashboard', impact: 'degraded' }],
    statusPageUrl: 'https://status.app.com',
  })),
  ...
  Referral_reward: renderEmail(Referral.component({
    name: 'Bob', event: 'reward_credited', referrerName: 'Bob',
    actionUrl: 'https://app.com/dashboard', reward: '$20 off',
  })),
...
describe('WCAG AA contrast — all templates', () => {
  for (const [templateName, html] of Object.entries(templates)) {
    it(`${templateName}: inline text colors ≥ 4.5:1 on card background (#ffffff)`, () => {
      ...
      expect(
        failures,
        `WCAG AA failures in ${templateName}: ${failures.join(', ')}`,
      ).toHaveLength(0);
    });
  }
});
