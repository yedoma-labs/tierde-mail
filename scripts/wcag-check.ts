...
const templates: Record<string, string> = {
  ...
  MaintenanceNotification_started: renderEmail(
    MaintenanceNotification.component({
      type: 'started',
...
for (const [name, html] of Object.entries(templates)) {
  ...
  if (failures.length === 0) {
    ...
    for (const { color, ratio } of failures) {
      console.log(
        `    ${RED}${color}${RESET}  ${ratio.toFixed(2)}:1  ${YELLOW}(need ${WCAG_AA}:1)${RESET}`,
      );
    }
  }
}
...
if (totalFailures === 0) {
  ...
  console.log(
    `${RED}${BOLD}${totalFailures} failure(s) across ${totalTemplates - passedTemplates} template(s)${RESET}`,
  );
  process.exit(1);
}
